...
output "load_balancer_arns" {
  ...
  value       = aws_lb_target_group.target_group.load_balancer_arns
}

output "tags_all" {
  description = "Map of all tags assigned to the target group, including those inherited from provider default_tags."
  value       = aws_lb_target_group.target_group.tags_all
}
